...
def _memory_config():
    ...
    if not api_key:
        print(
            "Warning: CML_API_KEY is not set. If the server requires authentication, requests might fail."
        )
...
async def bulk_ingest(document_text: str, session_id: str):
    ...
    async with AsyncCognitiveMemoryLayer(**config) as memory:
        ...
        for i, chunk in enumerate(chunks, 1):
            tasks.append(ingest_chunk(memory, chunk, session_id, semaphore, i, len(chunks)))
...
def main():
    ...
    """
        * 10
    )  # Multiply to simulate a larger document
