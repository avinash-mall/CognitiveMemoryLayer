 ...
 * Displays KPI cards, charts, system health,
 * reconsolidation status, and request sparkline.
 */

import { getOverview, getTimeline, getComponents, getLabile, getRequestStats } from '../api.js';
...
export async function renderOverview({ tenantId } = {}) {
    ...
    try {
        ...
        const [overview, timeline, components, labile, reqStats] = await Promise.all([
        ...
        el.innerHTML = buildOverviewHTML(overview, timeline, components, labile, reqStats);
...
function buildOverviewHTML(overview, timeline, components, labile, reqStats) {
    ...
    return `
        ...
        <div class="kpi-grid">
            ...
            ${kpiCard('Storage', formatMB(overview.estimated_size_mb), 'Estimated on disk')}
